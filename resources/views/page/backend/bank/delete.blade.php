    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title"
                id="bankdeleteLargeModalLabel{{ $bankdata->unique_key }}">
                Destroy Unit</h5>
        </div>
        <div class="modal-body px-4 py-5 text-center">
            ...
            <p class="text-muted font-size-16 mb-4">Please confirm that you wish to
                remove the Bank Details.</p>

            <div class="hstack gap-2 justify-content-center mb-0">
                <form autocomplete="off" method="POST"
                    action="{{ route('bank.delete', [$bankdata->unique_key]) }}">
                    @method('PUT')
                    @csrf
                    <button type="submit" class="btn btn-danger">Yes, Delete it</button>
                </form>
                <a href="{{ route('bank.index') }}"><button type="button" class="btn btn-secondary"
