            ...
            <div class="page-btn">
                <div style="display:flex;">
                    ...
                    @if ($last_word != 'supplier')
                        <a href="/pdf_export/{{ $last_word }}" class="badges bg-lightgrey btn btn-added">PDF</a>
                    @else
                        <a href="/allpdf_export" class="badges bg-lightgrey btn btn-added">PDF</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-2 col-sm-4 col-6" style="width: 12%">
                <a href="{{ route('supplier.index') }}" style="color: black">
                    <div class="dash-widget"
                        @if ($pos == 'supplier') style="border-color:red; background-color: red; margin-bottom:18px;" @endif>
                                        ...
                                        <ul class="list-unstyled hstack gap-1 mb-0">
                                            ...
                                            <li>
                                                <a href="#edit{{ $suppliertdata['unique_key'] }}" data-bs-toggle="modal"
                                                    ...
                                                    data-bs-target=".supplieredit-modal-xl{{ $suppliertdata['unique_key'] }}"
                                                    class="badges bg-lightyellow" style="color: white">Edit</a>
                                            </li><br />
                                            <li>
                                                <a href="#delete{{ $suppliertdata['unique_key'] }}" data-bs-toggle="modal"
                                                    data-id="{{ $suppliertdata['unique_key'] }}"
                                                    data-bs-target=".supplierdelete-modal-xl{{ $suppliertdata['unique_key'] }}"
                                                    class="badges bg-lightgrey" style="color: white; background-color:brown">Delete</a>
                                            </li>
                                            @if ($suppliertdata['LastPattityal'] != '')
                                                <li>
                                                    <a class="badges bg-lightred"
                                                        style="color: white">{{ date('d-m-Y', strtotime($suppliertdata['LastPattityal'])) }}</a>
                                                </li>
                                            @endif
                                        </ul>
                                    </td>
