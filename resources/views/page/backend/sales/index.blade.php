    ...
    <div class="content">
        <div class="page-header">
            ...
            <div style="display: flex;">
                ...
                @if ($last_word == 'sales')
                    <a href="/salesindex_pdfexport/{{ $today }}" target="_blank" class="btn btn-warning">PDF
                        Export</a>
                @else
                    <a href="/salesindex_pdfexport_branchwise/{{ $last_word }}/{{ $today }}" target="_blank"
        ...
        <div class="row py-2" style="margin-bottom:10px;">
            ...
            <div class="col-lg-2 col-sm-2 col-4" style="width: 12%">
                <a href="{{ route('sales.index') }}">
                    <div class="dash-widget"
                        @if ($pos == 'sales') style="border-color:red; background-color: red; margin-bottom:18px;" @endif>
            ...
            @foreach ($allbranch as $keydata => $allbranches)
                <div class="col-lg-2 col-sm-2 col-4" style="width: 12%">
                    ...
                    {{-- <a href="#todaystock{{ $allbranches->id }}" data-bs-toggle="modal"data-id="{{ $allbranches->id }}"
                        data-bs-target=".todaystock-modal-xl{{ $allbranches->id }}" class="btn btn-added btn-primary "
                        style="color:white;background-color: #dfd8f0;font-size: 13px;font-weight: 600;">Available Stock</a> --}}

                    <div class="modal fade todaystock-modal-xl{{ $allbranches->id }}" tabindex="-1"role="dialog"
                                    ...
                                    <td style="text-transform: uppercase;">
                                        @foreach ($Sales_datas['sales_terms'] as $index => $terms_array)
                                            @if ($terms_array['sales_id'] == $Sales_datas['id'])
                                                ...
                                                {{ $terms_array['kgs'] }}{{ $terms_array['bag'] }}-{{ $terms_array['price_per_kg'] }},<br />
                                            @endif
                                        @endforeach
                                    </td>
                                            ...
                                            <li>
                                                ...
                                                @elseif ($Sales_datas['status'] == 3)
                                                    <a href="{{ route('sales.print_view', ['unique_key' => $Sales_datas['unique_key']]) }}"
                                                        class="badges"
                                                        style="color: white;background: #622222;">Generated</a>
                                                @endif
                                            </li>
