            ...
            <div class="page-btn">
                <div style="display:flex;">
                    {{-- <input type="button" style="margin-right:10px"
                        class="btn btn-lightgreen waves-effect waves-light btn-added badges bg-green" id="viewtotal"
                        value="View Total"> --}}
                    <button type="button" style="margin-right:10px"
                        class="btn btn-primary waves-effect waves-light btn-added" data-bs-toggle="modal"
                        data-bs-target=".cusomer-modal-xl">Add Customer</button>
                    @if ($last_word != 'customer')
                        <a href="/customerpdf_export/{{ $last_word }}" class="badges bg-lightgrey btn btn-added">PDF</a>
                    @else
                        <a href="/allbranchpdf_export" class="badges bg-lightgrey btn btn-added">PDF</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-2 col-sm-4 col-6" style="width: 12%">
                <a href="{{ route('customer.index') }}" style="color: black">
                    <div class="dash-widget"
                        @if ($pos == 'customer') style="border-color:red; background-color: red; margin-bottom:18px;" @endif>
                    ...
                    <table class="table  customerdatanew">
                        ...
                        <tbody>
                            @foreach ($customerarr_data as $keydata => $customertdata)
                                        ...
                                        <ul class="list-unstyled hstack gap-1 mb-0">
                                            ...
                                            <li>
                                                <a href="#edit{{ $customertdata['unique_key'] }}" data-bs-toggle="modal"
                                                    ...
                                                    data-bs-target=".cusomeredit-modal-xl{{ $customertdata['unique_key'] }}"
                                                    class="badges bg-lightyellow" style="color: white">Edit</a>
                                            </li>
                                            <li>
                                                <a href="#delete{{ $customertdata['unique_key'] }}" data-bs-toggle="modal"
                                                    data-id="{{ $customertdata['unique_key'] }}"
                                                    data-bs-target=".cusomerdelete-modal-xl{{ $customertdata['unique_key'] }}"
                                ...
                                <div class="modal fade cusomeredit-modal-xl{{ $customertdata['unique_key'] }}"
                                    tabindex="-1" role="dialog" data-bs-backdrop="static"
                                    aria-labelledby="customereditLargeModalLabel{{ $customertdata['unique_key'] }}"
                                    aria-hidden="true">
                                    @include('page.backend.customer.edit')
                                </div>


                                <div class="modal fade customercheckbalance-modal-xl{{ $customertdata['unique_key'] }}"
                                    tabindex="-1" role="dialog" data-bs-backdrop="static"
                                    aria-labelledby="customercheckbalanceLargeModalLabel{{ $customertdata['unique_key'] }}"
                                    aria-hidden="true">
